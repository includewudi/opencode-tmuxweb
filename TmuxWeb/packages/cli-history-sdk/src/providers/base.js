 ...
 *
 *   enabled: boolean | getter returning boolean,
 *     Whether this provider is operational (DB exists, deps available, etc.).
 *     Prefer a dynamic getter so the status reflects runtime state.
 *
 *   listSessions(opts) → { sessions: SessionSummary[], total: number },
 *     opts: { limit?: number, offset?: number, search?: string, directory?: string }
 ...
 *   error: { name, message, statusCode, isRetryable } | null,
 *   timeCreated: number,
 *   timeUpdated: number,
 *   parts: PartRecord[],
 * }
