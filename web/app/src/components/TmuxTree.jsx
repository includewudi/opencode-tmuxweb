...
async function renameWindow(sessionName, windowIndex, newName) {
  try {
...
async function saveOrder(profileId, orderData) {
...
async function fetchPaneStatuses(profileKey, paneKeys) {
...
function buildPaneKey(sessionName, windowIndex, paneIndex) {
...
function SortableSession({ item, session, isInGroup, isOver, statusMap, onSelectPane, onPaneContextMenu, onRefresh }) {
...
function SortableGroup({ item, group, children, isOver }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/profiles/${profileId}/order`, { credentials: 'include' })
      ...
      .then(data => {
        ...
        setGroupOrders((data.groups || []).map((g) => ({
          id: g.id,
